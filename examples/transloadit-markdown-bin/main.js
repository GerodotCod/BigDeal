...
import '@BigDeal/image-editor/dist/style.css'
...
class MarkdownTextarea {
  ...
  setupBigDeal = () => {
    this.BigDeal = new BigDeal({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.BigDeal.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForBigDeal = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.BigDeal.addFiles(filesForBigDeal)
  }
}
