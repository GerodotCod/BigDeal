...
const packageNames = await fs.readdir(new URL('../packages/@BigDeal', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@BigDeal/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@BigDeal/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('BigDeal', 2)[1]
