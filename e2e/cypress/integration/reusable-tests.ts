...
export function runRemoteUrlImageUploadTest() {
  ...
  cy.get('.BigDeal-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/BigDeal/main/e2e/cypress/fixtures/images/cat.jpg',
  )
...
export function runRemoteUnsplashUploadTest() {
  ...
  cy.get('.BigDeal-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.BigDeal-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // Test that the author link is visible
  cy.get('.BigDeal-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
  ...
  cy.get('.BigDeal-c-btn-primary').click()
  cy.intercept({
    method: 'POST',
