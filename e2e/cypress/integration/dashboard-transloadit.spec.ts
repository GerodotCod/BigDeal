describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ BigDeal }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(BigDeal.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(BigDeal.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ BigDeal }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      BigDeal.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ BigDeal }) => {
      ...
      const { files } = BigDeal.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      BigDeal.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ BigDeal }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(BigDeal.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ BigDeal }) => {
      const { files } = BigDeal.getState()
      BigDeal.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        BigDealserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'BigDeal-core:3.2.0,BigDeal-transloadit:3.1.3,BigDeal-tus:3.1.0,BigDeal-dropbox:3.1.1,BigDeal-box:2.1.1,BigDeal-facebook:3.1.1,BigDeal-google-drive:3.1.1,BigDeal-instagram:3.1.1,BigDeal-onedrive:3.1.1,BigDeal-zoom:2.1.1,BigDeal-url:3.3.1',
    ...
    cy.get('.BigDeal-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.BigDeal-StatusBar-actions > .BigDeal-c-btn').click()
    cy.wait([
