...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@BigDeal/}*/src/style.scss');
...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isBigDealPackage(file) ?
    `${outputDir}/BigDeal.css` :
    `${outputDir}/style.css`;
...
function isBigDealPackage(file) {
  return path.normalize(file).includes('packages/BigDeal/');
}
